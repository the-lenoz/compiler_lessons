...
def process_expr(expr: Expr | None, f_gen: FuncCodeGenerator):
    ...
    if isinstance(expr, BinOP):
        ...
        process_expr(expr.array_name, f_gen)
        f_gen.emit_push("rax")

        process_expr(expr.index, f_gen)
        f_gen.emit_reg_assign("rbx", 8)
        f_gen.emit_mul()
...
def process_stmt(statement: Stmt, f_gen: FuncCodeGenerator):
    ...
    match statement:
        ...
        case Assignment(lvalue, expr):
            if isinstance(lvalue, Id):
                process_expr(expr, f_gen)
                f_gen.emit_store_val_to_var(lvalue.value)
            elif isinstance(lvalue, ArrayAccess):
                f_gen.emit_push("rdi")

                process_expr(lvalue.array_name, f_gen)
                f_gen.emit_push("rax")

                process_expr(lvalue.index, f_gen)
                f_gen.emit_reg_assign("rbx", 8)
                f_gen.emit_mul()
...
def process_f_def(definition: FDef, codegen: CodeGenerator):
    ...
    f_gen = FuncCodeGenerator(definition.decl.name.value, codegen)
